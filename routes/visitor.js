...
router.get('/temp-reject-test/:id', (req, res) => {
    const visitorId = req.params.id;
    ...
    db.query(sql, [reason, visitorId], (err, result) => {
        if (err) {
            return res.status(500).json({ error: 'DB update error', details: err.message, stack: err.stack });
        }

        const getVisitorSql = 'SELECT name, email FROM visitors WHERE id = ?';
        db.query(getVisitorSql, [visitorId], (getErr, getRes) => {
            if (getErr) {
                return res.status(500).json({ error: 'DB select error', details: getErr.message });
            }
            if (getRes.length === 0) {
            ...
            sendVisitorRejectionEmail(visitor.email, visitor.name, reason)
                .then(() => {
                    res.json({ success: true, message: 'Rejection email sent successfully', visitor });
                })
                .catch(mailErr => {
                    res.status(500).json({ error: 'Mail send error', details: mailErr.message, stack: mailErr.stack });
                });
        });
    });
});
